...
var is_held_down : bool = false:
	set(val):
		...
		if is_held_down and not val:
			if not is_rotating: parent.rotation = initial_parent_rotation
...
func _ready() -> void:
	...
	start_parent_alpha = parent.modulate.a
...
func _process(delta: float) -> void:
	print(mouse_in_area, is_held_down)
	...
	if is_held_down:
		...
		parent.position.x = clamp(parent.position.x, 0.0, DisplayServer.window_get_size().x)
		parent.position.y = clamp(parent.position.y, 0.0, DisplayServer.window_get_size().y)
		
		if visible_on_drag != null:
			visible_on_drag.show()
	
	elif visible_on_drag != null:
		visible_on_drag.hide()
	
	


func _input(event: InputEvent) -> void:
	if mouse_in_area or is_held_down:
