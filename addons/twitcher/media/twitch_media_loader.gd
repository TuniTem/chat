...
func _request_cheermote(cheer_tier: TwitchCheermote.Tiers, cheermote: TwitchCheermoteDefinition) -> BufferedHTTPClient.RequestData:
	return null
	#if not cheer_tier.images: return null
	#prints(cheermote.theme, cheermote.type, cheermote.scale) 
	#prints(typeof(cheer_tier.images), cheer_tier.images, )
	var img_path : String = cheer_tier.images[cheermote.theme]   [&"animated_format"]   [cheermote.scale]
